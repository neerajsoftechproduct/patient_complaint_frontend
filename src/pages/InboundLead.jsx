...
import { useGetInboundLeadsForAgentQuery } from '../stores/services/patientApi';
...
const InboundLead = () => {
  ...
  const [toDisplayLeads, setToDisplayLeads] = useState([])
  const { data: defaultLeads, refetch } = useGetInboundLeadsForAgentQuery(
    user?.uID ?
      {
        AgentID: user.uID
      }
  ...
  useEffect(() => {
    if (defaultLeads?.data) {
      setToDisplayLeads(defaultLeads?.data)
    }
  }, [defaultLeads])
  const paginatedData = toDisplayLeads?.slice(
    page * rowsPerPage,
      ...
      <Grid container spacing={2} mt={2} border="1px solid #ddd" borderRadius={2} >
        <Grid size={12}>
          <Box sx={{
            ...
            }} >Total Leads : {toDisplayLeads?.length || 0}</Typography>
                        ...
                        paginatedData?.map((item, index) => {
                          return (
                            <TableRow key={index} >
                              ...
                              <TableCell sx={tc}>{item?.AgentName}</TableCell>
                              <TableCell sx={tc}>{item?.disposition_code}</TableCell>
                              <TableCell sx={tc}>{item?.FollowupType || 'N/A'}</TableCell>
                              ...
                              <TableCell sx={tc}>{item?.callbk_date || 'N/A'}</TableCell>
                              <TableCell sx={tc}>
